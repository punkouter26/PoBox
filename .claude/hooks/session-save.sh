...
# git log fails on a repo with zero commits; capture it separately so the
# fallback can't concatenate with jq's output (two JSON texts break --argjson)
COMMIT_LINES=$(git log --oneline -3 2>/dev/null || true)
RECENT_COMMITS=$(printf '%s' "$COMMIT_LINES" | jq -Rs 'split("\n") | map(select(length > 0))' 2>/dev/null) || RECENT_COMMITS='[]'
...
# Read a state file only if it holds a single valid JSON text (empty or
# malformed files would break --argjson)
_read_json_or_empty() {
    local out
    out=$(jq -ce . "$1" 2>/dev/null) && [ -n "$out" ] && printf '%s' "$out" || printf '{}'
}

# Gather verification state if available
VERIFICATION="{}"
if [ -f "${UNITY_HOOK_STATE_DIR}/verify-state.json" ]; then
    VERIFICATION=$(_read_json_or_empty "${UNITY_HOOK_STATE_DIR}/verify-state.json")
fi

# Gather plan state if available
PLAN="{}"
if [ -f "${UNITY_HOOK_STATE_DIR}/plan-state.json" ]; then
    PLAN=$(_read_json_or_empty "${UNITY_HOOK_STATE_DIR}/plan-state.json")
fi

# Gather agent context if available
AGENT_CONTEXT="{}"
if [ -f "${UNITY_HOOK_STATE_DIR}/agent-context.json" ]; then
    AGENT_CONTEXT=$(_read_json_or_empty "${UNITY_HOOK_STATE_DIR}/agent-context.json")
fi
